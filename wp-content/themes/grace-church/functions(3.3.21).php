...
if($_POST['is_loggedin'] ==1) {
    echo "select * from " . $wpdb->prefix . "cfcms_directory where username ='" . $_POST['log'] . "'";
    exit;
    $cfcms_info = $wpdb->get_row("select * from " . $wpdb->prefix . "cfcms_directory where username ='" . $_POST['log'] . "'", 'ARRAY_A');
    print_r($cfcms_info);
    exit;
    if(!empty($cfcms_info)) {
    ...
    if(!empty($cfcms_info))
    {
        ...
        $cfcms_info = $wpdb->get_row("select * from " . $wpdb->prefix . "cfcms_directory where username ='" . $_POST['log'] . "' and password ='" . $_POST['pwd'] . "' and status='Active'", 'ARRAY_A');
...
function themeslug_enqueue_script() {
    ...
    //echo "$('#login_form').ajaxForm({url: '".site_url()."', type: 'post'});";
    //echo "$('#submit_button').on('click', function() { $.post('".site_url()."', $('#login_form').serialize());});";
    echo "$('#submit_button').on('click', function() { $.post('".site_url()."', $.param($('#login_form').serializeArray()), function(data) {";
    echo " if(data == 'login not successful') {jQuery('#password_failed').css('display','none');jQuery('#login_failed').css('display','block')} else if(data == 'password not successful') {jQuery('#login_failed').css('display','none');jQuery('#password_failed').css('display','block')} else {/*window.location.replace('".site_url()."/cfcms-directory');*/ alert(data)} });";
...
if(!empty($_GET['csv_download']) && $_GET['csv_download']=='yes') {
    ...
    header("Content-Disposition: attachment; filename=CFCMS_GROUPBUYINGINFO_".date('m-d-Y').".csv");
    ...
    foreach ($cfcms_lists as $row) {
        ...
        /*unset($row['name']);
        unset($row['specialty']);*/
        unset($row['practice_name']);
        unset($row['image']);
        unset($row['bg_image']);
        unset($row['manager_name']);
        unset($row['office_address1']);
        unset($row['office_address2']);
        unset($row['office_text']);
        unset($row['office_city']);
        unset($row['office_state']);
        unset($row['office_zip']);
        unset($row['office_phone']);
        unset($row['office_fax']);
        unset($row['office_email']);
        unset($row['office_direction']);
        unset($row['website']);
        unset($row['nurse_name']);
        unset($row['nurse_phone']);
        unset($row['nurse_email']);
        unset($row['accept_medicare']);
        unset($row['hospital_affiliation']);
        unset($row['medical_school']);
        unset($row['residency']);
        unset($row['internship']);
        unset($row['board_certification']);
        unset($row['biography']);
        unset($row['comment']);
        unset($row['created']);
        unset($row['updated']);
        if($i==1) {
            ...
            fputcsv($handle, $arr);   // direct to buffered output
        }
        fputcsv($handle, $row);   // direct to buffered output
...
function cfcmslist_edit()
{
	global $wpdb;
        ...
        if (!empty($_REQUEST['femail'])) {
            $sql_str .= " and username like '%" . $_REQUEST['femail'] . "%' ";
        }
        if (!empty($_REQUEST['fname'])) {
            $sql_str .= " and name like '%" . $_REQUEST['fname'] . "%' ";
        }
        ...
        <div class="wrap">
            ...
            <form method="post" action="" enctype="multipart/form-data">
                ...
                <br/>
                <table width="100%" cellpadding="5" cellspacing="1" border="0" style="border: 2px solid #e3e3e3">
                    <tr>
                        <td style="border-right: 2px solid #e3e3e3"><strong>CFCMS Members CSV File:</strong></td>
                        <td style="border-right: 2px solid #e3e3e3"><b>Email:</b> &nbsp;
                            <input type="text" name="fname" placeholder="Enter name" style="width: 300px"
                                   value="<?php echo $_REQUEST['fname'] ?>"/>&nbsp;<br/><br/>
                            <b>Email:</b> &nbsp;
                    ...
                    <tr>
                        <td style="border-right: 2px solid #e3e3e3"><input type="file" name="fupload"></td>
                        <td style="border-right: 2px solid #e3e3e3"><input type="submit" value="Search" class="button button-primary" style="cursor: pointer"/></td>
                        <td></td>
                    </tr>
                    <tr>
                        ...
                        <td style="border-right: 2px solid #e3e3e3">&nbsp;</td>
                ...
                <tr>
                    ...
                    <td width="80%">
                        <select name="status" style="width: 50%;">
                            ...
                            <option value="Active" <?php if($lead_info['status'] == 'Active'): ?> selected <?php endif; ?>>Active</option>
                            <option value="Inactive" <?php if($lead_info['status'] == 'Inactive'): ?> selected <?php endif; ?>>Inactive</option>
                        </select>
                    </td>
                </tr>
