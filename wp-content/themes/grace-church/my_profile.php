...
/**
 * Template Name: My Bestof Profile Template
...
if(!empty($_GET['email'])) {
    $userinfo = $mydb->get_row($mydb->prepare("SELECT id FROM bestof_users WHERE email = '" . $_GET['email'] . "' and id<>" . $_SESSION['ses_user']['id']));
...
$sql = "SELECT title FROM `wp_bestof_categories` order by title asc";
$catLists = $mydb->get_results($sql,'ARRAY_A');
...
if(!empty($_SESSION['ses_user'])):
    ...
    if(!empty($_POST)) {
        ...
        $sm_imge_sql_str = "";
        if (!empty($image_name)) {
            $sm_imge_sql_str = "`image` = '" . $image_name . "',";
        }

        $mydb->query("UPDATE `bestof_users` SET `fname` = '" . $_POST['fname'] . "', `lname` = '" . $_POST['lname'] . "', " . $pass_str . " `email` = '" . $_POST['email'] . "', `cell` = '" . $_POST['cell'] . "', " .$sm_imge_sql_str . $bg_imge_sql_str . "  `title` = '" . $_POST['title'] . "',  `office_address1` = '" . $_POST['office_address1'] . "', `office_address2` = '" . $_POST['office_address2'] . "', `office_city` = '" . $_POST['office_city'] . "', `office_state` = '" . $_POST['office_state'] . "', `office_zip` = '" . $_POST['office_zip'] . "', `office_phone` = '" . $_POST['office_phone'] . "', `office_fax` = '" . $_POST['office_fax'] . "', `office_email` = '" . $_POST['office_email'] . "', `website` = '" . $_POST['website'] . "', `cat_list` = '" . implode(',', $_POST['cat_list']) . "', `biography` = '" . $_POST['biography'] . "', `year_in_business` = '" . $_POST['year_in_business'] . "', `cat_list` = '" . json_encode($_POST['cat_list']) . "', `updated` = '" . time() . "' WHERE `id` = " . $_SESSION['ses_user']['id']);

    }

    $cfcms_info = $mydb->get_row("select * from bestof_users where `id` = ".$_SESSION['ses_user']['id'],'ARRAY_A');
	...
	<div class="sc_contact_form_wrap" style="margin-bottom:40px">
        ...
        <a style="color:#fff;margin: 10px 0px;display: inline-block" href="<?php echo site_url() ?>/my-profile"><button style="background-color: #003D89">My Profile</button></a>
        <a style="color:#fff;margin: 10px 0px;display: inline-block" href="<?php echo site_url() ?>/appointment-requests"><button style="background-color: #FF005A">My Appointments</button></a>
