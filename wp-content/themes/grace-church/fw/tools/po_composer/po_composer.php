...
// Theme init
if (!function_exists('grace_church_po_composer_theme_setup')) {
	add_action( 'grace_church_action_after_init_theme', 'grace_church_po_composer_theme_setup' );		// Fire this action after load theme options
	function grace_church_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && grace_church_get_theme_option('admin_po_composer')=='yes') {
			new grace_church_po_composer();
		}
	}
}


class grace_church_po_composer {
	...
	function admin_menu_item() {
		...
		//add_theme_page( esc_html__('PO Composer', 'grace-church'), esc_html__('PO Composer', 'grace-church'), 'edit_theme_options', 'po_composer', array($this, 'build_page'));

		// In this case menu item is add in admin menu 'Tools'
		add_management_page( esc_html__('PO Composer', 'grace-church'), esc_html__('PO Composer', 'grace-church'), 'manage_options', 'po_composer', array($this, 'build_page'));
	}
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			grace_church_enqueue_style('po-composer-style', grace_church_get_file_url('tools/po_composer/po_composer.css', array(), null));
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			...
			grace_church_enqueue_script('po-composer-script', grace_church_get_file_url('tools/po_composer/po_composer.js', array('jquery'), null, true));
		}
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'. esc_html__('Upload .po-file', 'grace-church').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'. esc_html__('Edit .po-text', 'grace-church').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? esc_html__('Parent languages', 'grace-church') : esc_html__('Languages', 'grace-church')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'. esc_html__('Child languages', 'grace-church').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'. esc_html__('- Select merge source -', 'grace-church').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'. esc_html__('Upload .po-file', 'grace-church').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'. esc_html__('Edit .po-text', 'grace-church').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? esc_html__('Parent languages', 'grace-church') : esc_html__('Languages', 'grace-church')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'. esc_html__('Child languages', 'grace-church').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

        $nonce = wp_create_nonce( get_admin_url() );
		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php esc_html_e('Grace-Church .PO-files Composer', 'grace-church'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php esc_html_e('Edit .po-file:', 'grace-church'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php esc_html_e('Select .po-file:', 'grace-church'); ?></label>
							...
							<div class="po_composer_field po_composer_editor">
								...
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php esc_html_e('Move translated strings down', 'grace-church'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php esc_html_e('Plain text', 'grace-church'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php esc_html_e('Strings editor', 'grace-church'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php esc_html_e('Merge with .po-file:', 'grace-church'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php esc_html_e('Select .po-file:', 'grace-church'); ?></label>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? esc_html__('Upload', 'grace-church') : esc_html__('Update', 'grace-church')); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php esc_html_e('Download .PO-file', 'grace-church'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php esc_html_e('Download .MO-file', 'grace-church'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
		...
		if (!empty($this->po_src)) {
            if ( !isset($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], get_admin_url() ) ) {
				...
				do {
					...
					if (($po_data = $this->parse_po($po_text))===false) {
						$this->error = esc_html__('Error parsing new .po-file!', 'grace-church');
						break;
					}
					if ($this->po_src2 != '') {
						...
						if (($po_data2 = $this->parse_po($po_text2))===false) {
							$this->error = esc_html__('Error parsing merging .po-file!', 'grace-church');
							break;
						}
					}
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					grace_church_fpc($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					grace_church_fpc($mo_temp, $this->mo_text);
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						grace_church_fpc($po_file, $this->po_text);
						$this->success = sprintf( esc_html__('Language file "<b>%s.po</b>" was updated!', 'grace-church'), $name);
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							grace_church_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>'.sprintf( esc_html__('Language file "<b>%s.mo</b>" was generated!', 'grace-church'), $name);
						}
					} else {
						$tjis->error_update = sprintf( esc_html__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'grace-church'), $name);
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.($suffix)]['tmp_name']) && file_exists($_FILES['po_file'.($suffix)]['tmp_name']) ? grace_church_fga($_FILES['po_file'.($suffix)]['tmp_name']) : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf( esc_html__('Error uploading or Empty .po-file: %s', 'grace-church'), $_FILES['po_file'.($suffix)]['tmp_name']);
					break;
				}
			// or get content from textarea
				...
				if (!empty($_POST['po_text'.($suffix)]))
					...
					$rez['error'] = esc_html__('Empty textarea with .po-file content!', 'grace-church');
					break;
				}
			// or load file from 'languages' folder
				...
				$name = grace_church_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? grace_church_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf( esc_html__('Error loading or Empty .po-file: %s', 'grace-church'), ($dir).'/'.($name).'.po');
					break;
				}
