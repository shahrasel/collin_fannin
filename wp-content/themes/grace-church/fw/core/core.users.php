...
if (!function_exists('grace_church_add_fields_in_user_profile')) {
	...
	?>
		<h3><?php esc_html_e('User Position', 'grace-church'); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="user_position"><?php esc_html_e('User position', 'grace-church'); ?>:</label></th>
				<td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
					<span class="description"><?php esc_html_e('Please, enter your position in the company', 'grace-church'); ?></span>
				</td>
			</tr>
		</table>
	
		<h3><?php esc_html_e('Social links', 'grace-church'); ?></h3>
		<table class="form-table">
		<?php
		...
		if (is_array($social_list) && count($social_list) > 0) {
			foreach ($social_list as $soc) {
				...
				if (!empty($sn)) {
					?>
					<tr>
						<th><label for="user_<?php echo esc_attr($sn); ?>"><?php echo trim(grace_church_strtoproper($sn)); ?>:</label></th>
						<td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
							<span class="description"><?php echo sprintf( esc_html__('Please, enter your %s link', 'grace-church'), grace_church_strtoproper($sn)); ?></span>
						</td>
					</tr>
					<?php
				}
