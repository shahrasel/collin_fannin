...
if (!function_exists('grace_church_taxonomy_add_options_column')) {
	//add_filter('manage_edit-taxonomy_columns',	'grace_church_taxonomy_add_options_column', 9);
...
if (!function_exists('grace_church_taxonomy_fill_options_column')) {
	//add_filter('manage_taxonomy_custom_column',	'grace_church_taxonomy_fill_options_column', 9, 3);
...
//add_action( 'taxonomy_edit_form_fields', 'grace_church_taxonomy_show_custom_fields', 10, 1 );
//add_action( 'taxonomy_add_form_fields', 'grace_church_taxonomy_show_custom_fields', 10, 1 );
if (!function_exists('grace_church_taxonomy_show_custom_fields')) {
	function grace_church_taxonomy_show_custom_fields($tax_obj = null) {
        ...
        echo '<input type="hidden" name="meta_box_taxonomy_nonce" value="'.esc_attr($GRACE_CHURCH_GLOBALS['admin_nonce']).'" />';
		...
		do_action('grace_church_action_taxonomy_before_show_meta_box', $tax_type, $tax_obj);
		...
		do_action('grace_church_action_taxonomy_after_show_meta_box', $tax_type, $tax_obj);
		?>
			</td>
		</tr>
		</table>
...
// Save the changes made on the taxonomy, using our callback function  
//add_action( 'edited_taxonomy', 'grace_church_taxonomy_save_custom_fields', 10, 1 );
//add_action( 'created_taxonomy', 'grace_church_taxonomy_save_custom_fields', 10, 1 );
if (!function_exists('grace_church_taxonomy_save_custom_fields')) {
	function grace_church_taxonomy_save_custom_fields( $term_id=0 ) {
		global $GRACE_CHURCH_GLOBALS;

		// verify nonce
        if (!isset($_POST['meta_box_taxonomy_nonce']) || !wp_verify_nonce($_POST['meta_box_taxonomy_nonce'], $GRACE_CHURCH_GLOBALS['admin_url'])) {
			return $term_id;
		}
