...
if (!function_exists('grace_church_attachment_view_compat')) {
	...
	function grace_church_attachment_view_compat($form_fields, $post=null) {
		...
		if (isset($form_fields['media_folder'])) {
			...
			$html = '';
			$html .= '<input type="text" class="text" id="media_folder_'.esc_attr($id).'" name="media_folder___new" value="" placeholder="' . esc_html__('Create new folder', 'grace-church') . '"' . ($readonly) . ' ' . ($aria_required) . ' />';
			if (is_array($terms) && count($terms) > 0) {
